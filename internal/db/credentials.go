...
func CreateCredential(
	...
	err = q.QueryRow(ctx, `
		INSERT INTO credentials (access_key_id, secret_ciphertext, secret_nonce, description, owner_user_id)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id::text, created_at`,
		accessKeyID, ciphertext, nonce, description, ownerUserID,
...
func LookupCredential(
	...
	err := q.QueryRow(ctx, `
		SELECT id::text, access_key_id, secret_ciphertext, secret_nonce,
		       description, owner_user_id::text, created_at, last_used_at, revoked_at
		FROM credentials
		WHERE access_key_id = $1`,
		accessKeyID,
	).Scan(&cred.ID, &cred.AccessKeyID, &ciphertext, &nonce,
		&cred.Description, &cred.OwnerUserID, &cred.CreatedAt, &cred.LastUsedAt, &cred.RevokedAt)
